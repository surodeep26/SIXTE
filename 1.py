...
class Simulation:
    def __init__(self, name, model_string="tbabs*bbodyrad",model_parameters=None):
        self.name = name
        self.model_string = model_string
        if not os.path.exists(self.name):
            os.makedirs(os.path.join(self.name, 'products'))
        create_model(self, modelstring=self.model_string, 
                     setPars=model_parameters,
                     save=f"{self.name}/Model_{model_string.replace("*","_")}")
...
s = Simulation('rxj186', model_parameters=(1,2,4))